...
## 多值参数说明
- **指标(metric)参数支持多选**：如果用户同时提到多个指标（如"销售额、销售量"），请将所有指标用顿号、连接，作为metric参数值传入，如 "销售额、销售量、订单数"。不要要求用户只选一个。
- 多个指标会在SQL中生成多个列，每个指标一列。
...
## 多值参数说明
- **指标(metric)参数支持多选**：如果用户同时提到多个指标（如"本月的销售额和销售量"），请将所有指标用顿号、连接作为metric参数值，如 "销售额、销售量"。不要要求用户只选一个。
- 对于趋势分析、同环比分析等复杂模板（包含窗口函数），仅支持单指标。
