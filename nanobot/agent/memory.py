...
class MemoryStore:
    ...
    def __init__(
        ...
        # Global-level paths
        self._global_dir = ensure_dir(self.memory_root / "_global")
        ...
        if memory_key:
            ...
            self._user_dir = ensure_dir(self.memory_root / "users" / safe_key)
...
class MemoryManager:
    ...
    def __init__(self, workspace: Path):
        ...
        self.memory_root = ensure_dir(workspace / "memory")
        self._global_dir = ensure_dir(self.memory_root / "_global")
        self._users_dir = ensure_dir(self.memory_root / "users")
    ...
    def set_user_memory(self, user_id: str, channel: str, content: str) -> None:
        ...
        memory_key = f"{user_id}:{channel}"
        safe_key = memory_key.replace(":", "_")
        user_dir = ensure_dir(self._users_dir / safe_key)
        memory_file = user_dir / "MEMORY.md"
        memory_file.write_text(content, encoding="utf-8")
